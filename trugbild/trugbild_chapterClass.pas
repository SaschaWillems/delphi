...
begin
  ...
  else if Decision.TimeLeft < -15 then
  begin
    DeathScene.LightFade := Abs(Decision.TimeLeft + 15) / 10;
    RealityFade          := 1 - (Decision.TimeLeft + 20) / 10;
  end
  else if Decision.TimeLeft < -10 then
    RealityFade  := 1 - (Decision.TimeLeft + 20) / 10// Show reality scene (TODO : with death hint, see above)

  else if Decision.TimeLeft < 0 then
    BlurStrength := Clamp(BlurStrength + TimeFactor * 0.15, 0, 15)// Start to blur sceen

  else if Decision.TimeLeft < 10 then
    if Decision.TimeLeft > 0 then
      Player.StressLevel := Clamp(1 + (10 - Decision.TimeLeft) * 0.4, 1, 4)// Increase stress level
  ;
end;
