...
uses
  ...
  XMLDoc, XMLIntf, bassSoundSystem, ShellAPI, ShlObj, ActiveX, VirtualFileSystem,

  TrugBild_Global, TrugBild_RealityScene, TrugBild_GameClass, TrugBild_DecisionClass,
  TrugBild_PlayerClass, TrugBild_DeathScene, TrugBild_MainMenu, TrugBild_EndingScene,
  TrugBild_ChapterClass, TrugBild_About;
...
type
  TStage = class
      ...
      Title     : String;
      TagLine		: String;
      procedure Update;           procedure Render(AFBO : TFrameBufferObject; AForColorPicking : Boolean);
...
begin
  ...
  if Game.State = gsMainMenu then
      ...
      glBegin(GL_QUADS);
        ...
        glTexCoord2f(GLForm.ClientWidth, 0);           glVertex3f(OrthoSize.x, 0, 0);
        glTexCoord2f(GLForm.ClientWidth, OrthoSize.y); glVertex3f(OrthoSize.x, OrthoSize.y, 0);
        glTexCoord2f(0, OrthoSize.y);                  glVertex3f(0, OrthoSize.y, 0);
      glEnd;
      ShaderManager.DisableShader;
...
begin
  case Key of
	...
	if DebugKeys then
      ...
      'l' : if GetCurrentDecision <> nil then GetCurrentDecision.Visual := visLadders;
      'h' : if GetCurrentDecision <> nil then GetCurrentDecision.Visual := visHoles;
      'd' : if GetCurrentDecision <> nil then GetCurrentDecision.Visual := visDoors;
...
//      'b' : Renderer.Blur := not Renderer.Blur;
      ...
      'n' : begin
              if not Stage.CurrentChapter.NextDecision then
                begin
                  Stage.NextChapter;
                end;
            end;
...
begin
  ...
  if AFBO = Renderer.FBOText then begin
    glMatrixMode(GL_PROJECTION);
    glLoadIdentity;
...
begin
  ...
  if AFBO = Renderer.FBO then
      ...
      TextureManager.SetBlending(bmBlend);
      glBegin(GL_QUADS);
        glColor4f(0.1, 0.1, 0.1, 1); glVertex3f(0,          0,   1);
        glColor4f(0.1, 0.1, 0.1, 1); glVertex3f(AFBO.Width, 0,   1);
        glColor4f(0.1, 0.1, 0.1, 0); glVertex3f(AFBO.Width, AFBO.Height*0.65, 1);
        glColor4f(0.1, 0.1, 0.1, 0); glVertex3f(0,          AFBO.Height*0.65, 1);
      glEnd;
      glColor3f(1, 1, 1);
    end;
...
begin
  ...
  if Length(GetCurrentDecision.Answers) = 3 then
    ...
    if (not AForColorPicking) and (AFBO = Renderer.FBO) then
      begin
        glBegin(GL_QUADS);
          glColor3f(0, 0, 0);          glVertex3f(-150,  CeilPos, -100);
          glColor3f(0.25, 0.25, 0.25); glVertex3f(-5000, CeilPos,  200);
          glColor3f(0.25, 0.25, 0.25); glVertex3f( 5000, CeilPos,  200);
          glColor3f(0, 0, 0);          glVertex3f( 150,  CeilPos, -100);
        glEnd;
      end;
        ...
        begin
          ...
          glPushMatrix;
            ...
            if (AFBO = Renderer.FBO) or (AFBO = Renderer.FBOColSel) then
                ...
                TextureManager.SetBlending(bmBlend);
                glBegin(GL_QUADS);
                  glTexCoord2f(0,0); glVertex3f(-HoleSize, CeilPos, -10+HoleSize);
                  glTexCoord2f(1,0); glVertex3f( HoleSize, CeilPos, -10+HoleSize);
                  glTexCoord2f(1,1); glVertex3f( HoleSize, CeilPos, -10-HoleSize);
                  glTexCoord2f(0,1); glVertex3f(-HoleSize, CeilPos, -10-HoleSize);
                glEnd;
            ...
            if (AFBO = Renderer.FBOEffects) and (GetCurrentDecision.Selection = i)  then
              begin
                ...
                glBegin(GL_QUADS);
                  glTexCoord2f(0,0); glVertex3f(-HoleSize*1.2, CeilPos, -6+HoleSize*1.75);
                  glTexCoord2f(1,0); glVertex3f( HoleSize*1.2, CeilPos, -6+HoleSize*1.75);
                  glTexCoord2f(1,1); glVertex3f( HoleSize*1.2, CeilPos, -6-HoleSize*1.75);
                  glTexCoord2f(0,1); glVertex3f(-HoleSize*1.2, CeilPos, -6-HoleSize*1.75);
                glEnd;
...
begin
  ...
  if Length(GetCurrentDecision.Answers) = 3 then
    ...
    if (not AForColorPicking) and (AFBO = Renderer.FBO) then
      begin
        glBegin(GL_QUADS);
          glColor3f(0, 0, 0);       glVertex3f(-150, 10, -100);
          glColor3f(0.4, 0.4, 0.4); glVertex3f(-5000, 10,  200);
          glColor3f(0.4, 0.4, 0.4); glVertex3f( 5000, 10,  200);
          glColor3f(0, 0, 0);       glVertex3f(150, 10, -100);
        glEnd;
      end;
      ...
      for i := 0 to High(GetCurrentDecision.Answers) do
        begin
          ...
          glPushMatrix;
            ...
            TextureManager.SetBlending(bmBlend);
            glBegin(GL_QUADS);
              glTexCoord2f(0,1); glVertex3f(-HoleSize, 10, -10+HoleSize);
              glTexCoord2f(1,1); glVertex3f( HoleSize, 10, -10+HoleSize);
              glTexCoord2f(1,0); glVertex3f( HoleSize, 10, -10-HoleSize);
              glTexCoord2f(0,0); glVertex3f(-HoleSize, 10, -10-HoleSize);
            glEnd;
            ...
            if (AFBO = Renderer.FBOEffects) and (GetCurrentDecision.Selection = i)  then
              begin
                ...
                TextureManager.BindTexture('radialgradientblack');
                glBegin(GL_QUADS);
                  glTexCoord2f(0,0); glVertex3f(-HoleSize*1.2, 10, -6+HoleSize*1.75);
                  glTexCoord2f(1,0); glVertex3f( HoleSize*1.2, 10, -6+HoleSize*1.75);
                  glTexCoord2f(1,1); glVertex3f( HoleSize*1.2, 10, -6-HoleSize*1.75);
                  glTexCoord2f(0,1); glVertex3f(-HoleSize*1.2, 10, -6-HoleSize*1.75);
                glEnd;
